...
def find_downstream_entities(graph: LineageGraph, entity_name: str, max_depth: int = 10) -> Set[str]:
    """
...
def visualize_lineage_mermaid(graph: LineageGraph, focus_entity: Optional[str] = None) -> str:
    ...
    if focus_entity:
        ...
        if node_id:
            ...
            edges_to_show = [e for e in graph.edges if e.from_node in related_ids and e.to_node in related_ids]
        else:
...
def visualize_lineage_graphviz(graph: LineageGraph, focus_entity: Optional[str] = None) -> str:
    ...
    for node in nodes_to_show.values():
        ...
        if node.type == NodeType.ENTITY:
            lines.append(f'    {node_id} [label="{node.name}", fillcolor="#e1f5ff", style="filled,rounded"];')
        elif node.type == NodeType.RELATION:
            lines.append(f'    {node_id} [label="{node.name}", shape=diamond, fillcolor="#fff9c4", style="filled"];')
        elif node.type == NodeType.SOURCE:
            lines.append(f'    {node_id} [label="{node.name}", shape=cylinder, fillcolor="#f3e5f5", style="filled"];')
